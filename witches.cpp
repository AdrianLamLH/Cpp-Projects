...
{
    int num_witches_at = 0;
    for (int i = 0; i < m_nWitches; i++)
    {
        if (m_witches[i]->col() == c && m_witches[i]->row() == r)
        {
            if (num_witches_at < 9)
            {
                num_witches_at++;
...
void City::display(string msg) const
{
    ...
    for (int i = 0; i < m_nWitches; i++)
    {
        if (m_witches[i] != nullptr)
        {
            char& gridChar = grid[m_witches[i]->row() - 1][m_witches[i]->col() - 1];
            if (gridChar == '.')
            {
                gridChar = 'W';
            }
            ...
            {
                char num_val[] = {'2','3','4','5','6','7','8','9' };
                gridChar = num_val[numWitchesAt(m_witches[i]->row(), m_witches[i]->col())-2];
            }
        }
    }
...
{
    // If MAX have already been added, return false.  Otherwise,
...
bool City::meltWitch(int r, int c)
{
    ...
    else
    {
        ...
        {
            if (m_witches[k]->col() == c && m_witches[k]->row() == r)
            {
                delete m_witches[k];
                for (int j = k; j < m_nWitches - 1; j++)
                {
...
bool City::moveWitches()
{
    ...
    {
        m_witches[k]->move();
        if (m_player->col() == m_witches[k]->col() && m_player->row() == m_witches[k]->row())
        {
            m_player->setDead();
        }
    }
